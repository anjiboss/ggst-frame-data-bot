...
function capitalize(word) {
  ...
  return word.slice(0, word.length - 1) + word.charAt(word.length-1).toUpperCase();
}
...
const dataFrame = (fileName, charInput, cb) => {
  const results = [];
    ...
    .on('end', () => {
      ...
      if (data === null){
        // No Move 
        return cb("```diff\n-Character: " + fileName.split("_")[0].split("/")[1] + " Don't Have That Move!```")
        
      }else {
        // console.log("outter", data)
        const dataTable = createDataTable(data, fileName.split("_")[0].split("/")[1])
        return cb(dataTable)
      }
    });
}
